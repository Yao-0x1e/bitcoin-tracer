...
def batch_get_raw_transactions(txids: List[str]) -> List[dict]:
...
def batch_get_block_hashes(block_nums: List[int]) -> List[dict]:
...
def batch_get_blocks(block_hashes: List[str], only_txids: bool = False) -> List[dict]:
    verbosity = 1 if only_txids else 2
...
def batch_get_block_headers(block_hashes: List[str]) -> List[dict]:
